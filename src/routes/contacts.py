...
# Entities
from src.models.entities.User import Admin, Default_user
...
def authentication(f):
    ...
    def wrapper(*args, **kwargs):
        ...
        if not info_user:
            abort(make_response('Неверный логин', 403))

        # создание экземпляра Admin или Default_user в зависимости от роли клиента
        if info_user[2] == 'a':
...
def show(user):
    ...
    for parametr in request.form:
        ...
        if parametr == 'typeSort':
            if not 'sort' in request.form:
                abort(make_response('Указан "typeSort" без указания параметра "sort"', 400))
            if not (request.form['typeSort'] == 'standart' or 'reverse'):
                return abort(
                    make_response('Недопустимое значение "typeSort". Укажите "standart" или "reverse".', 400)
                )
        dict_params[parametr] = request.form[parametr]
    data = user.show(
        dict_params['search'], dict_params['sort'], dict_params['typeSort'], dict_params['filters']
    )
    if data['err']:
        abort(make_response(data['err'], 500))
...
def add(user):
    ...
    for parametr in request.form:
        if parametr == 'phone':
            ...
            if err:
                abort(make_response(str(err), 400))
            continue
        if parametr == 'email':
...
def delete(user, contact_id):
    ...
    if (
        'irrevocable' in request.form and request.form['irrevocable']
    ):  # если запрашивается безвозвратное удаление
